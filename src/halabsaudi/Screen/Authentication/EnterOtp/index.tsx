...
const Otp: React.FC<OtpProps> = ({ route, navigation }) => {
  ...
  // 📋 Clipboard polling (auto-read OTP)
  useEffect(() => {
  ...
  // ✅ Check hala_permissions_asked — if never asked → show LocationDisclosure
  // LocationDisclosure will ask ALL permissions (notification + foreground + background) ONCE
  ...
  const saveUserData = async (token: string, user: any) => {
    try {
      ...
      console.log('✅ User data & token saved');
    } catch (e) {
  ...
  const confirmCode = async (code?: string) => {
    ...
    // 🔑 Dummy bypass for Apple reviewer
    if (isDummy) {
      if (pin !== '123456') {
      ...
      try {
        await AsyncStorage.multiSet([
          ...
          ['hala_user_backend', JSON.stringify({
      ...
      <ScrollView contentContainerStyle={styles.MainCont}>
        ...
        <View style={{ width: '100%', marginTop: 20 }}>
          <TextInput
            ...
            returnKeyType="done"
            onSubmitEditing={() => { if (otp.length === 6) confirmCode(); }}
            accessible
        ...
        {/* Resend — hidden for dummy session */}
        {!isDummy && (
          <View style={{ marginTop: 16, alignItems: 'center' }}>
            <TouchableOpacity
              ...
              <Text style={{
                color: resendCooldown > 0 || isResending ? Colors.Grey9 : Colors.Green,
